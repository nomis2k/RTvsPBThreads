...
void pb::Results::save() const
{
    ...
    h1 = _jet_flavor->convert("jet_flavor", "Flavor of the jets");
    h1->Write();
    ...
    h1 = _jet_x->convert("jet_x", "Jets x");
    h1->Write();

    h1 = _jet_y->convert("jet_y", "Jets y");
    h1->Write();

    h1 = _jet_z->convert("jet_z", "Jets z");
    h1->Write();

    // Muons
    //
    h1 = _muons->convert("muons", "Number of muons");
    h1->Write();

    h1 = _muon_e->convert("muon_e", "Muons e");
    h1->Write();

    h1 = _muon_px->convert("muon_px", "Muons px");
    h1->Write();

    h1 = _muon_py->convert("muon_py", "Muons py");
    h1->Write();

    h1 = _muon_pz->convert("muon_pz", "Muons pz");
    h1->Write();

    h1 = _muon_x->convert("muon_x", "Muons x");
    h1->Write();

    h1 = _muon_y->convert("muon_y", "Muons y");
    h1->Write();

    h1 = _muon_z->convert("muon_z", "Muons z");
    h1->Write();
}
